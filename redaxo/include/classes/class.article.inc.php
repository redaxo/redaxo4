...
class article
{
        ...
        function setArticleId($article_id)
        {
                ...
                if (!$REX[GG])
                {
                        ...
                        $this->ARTICLE->setQuery("select * from rex_article left join rex_category on rex_article.category_id=rex_category.id where rex_article.id='$article_id'");

                        if ($this->ARTICLE->getRows() == 1)
                        {
                                $this->template_id = $this->ARTICLE->getValue("rex_article.template_id");
                                $this->category_id = $this->ARTICLE->getValue("rex_article.category_id");
                                return TRUE;
                        }else
                        {
                                $this->article_id = 0;
        ...
        function sliceIn($slice_content)
        {
                for ($i=1;$i<11;$i++)
                {
                        ...
                        $media.= "<td><a href=javascript:openREXMedialist($i);><img src=pics/file_open.gif width=16 height=16 title='medienpool' border=0></a></td>";
                        ...
                        $media.= "<td><a href=javascript:openREXMedia($i);><img src=pics/file_show.gif width=16 height=16 title='medienpool' border=0></a></td>";
                        $media.= "</tr></table>";
