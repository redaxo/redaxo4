...
class rex_xform_com_auth_form_login extends rex_xform_abstract
{
	...
	function enterObject(&$email_elements,&$sql_elements,&$warning,&$form_output,$send = 0)
	{
		...
		$login = rex_request($REX['ADDON']['editme']['plugin_auth']['request']['name'],"string");
		...
		<p class="formtext form-com-auth-login formlabel-'.$this->getName().'" id="'.$this->getHTMLId().'">
			...
			<input type="text" class="text" name="'.$REX['ADDON']['editme']['plugin_auth']['request']['name'].'" id="el_'.$this->getId().'" value="'.htmlspecialchars(stripslashes($login)).'" />
		</p>
		';		
