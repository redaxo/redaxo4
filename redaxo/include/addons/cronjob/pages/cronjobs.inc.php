...
if ($func == '') 
{
  ...
  $list->setColumnFormat('environment', 'custom', 
    create_function( 
      ...
      'global $I18N;
       $list = $params["list"]; 
       if ($list->getValue("environment") == 0) 
         return $I18N->msg("cronjob_environment_both");
       if ($list->getValue("environment") == 1) 
         return $I18N->msg("cronjob_environment_frontend");
       if ($list->getValue("environment") == 2) 
         return $I18N->msg("cronjob_environment_backend");' 
    ) 
  );
...
} elseif ($func == 'edit' || $func == 'add') 
{
  ...
  $select->addOption($I18N->msg('cronjob_type_httprequest'),3);
  ...
  if (!empty($extensions)) 
  {
    ...
    $select =& $field->getSelect();
    $select->setSize(count($extensions));
  ...
  $select =& $field->getSelect();
  $select->setSize(3);
  $select->addOption($I18N->msg('cronjob_environment_both'),0);
  $select->addOption($I18N->msg('cronjob_environment_frontend'),1);
  $select->addOption($I18N->msg('cronjob_environment_backend'),2);
  if ($func == 'add')
