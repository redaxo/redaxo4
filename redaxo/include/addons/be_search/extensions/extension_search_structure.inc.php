...
//    $search->debugsql = true;
    ...
    {
      $message = rex_warning($I18N->msg('search_no_results'));
    }
  }
   ...
   '  <form action="index.php" method="post">
      <fieldset>
        ...
        <input type="hidden" name="a256_clang" value="'. $clang .'" />
	        ...
	        <label for="rex-a256-article-name">'. $I18N->msg('search_article_name') .'</label>
        	...
        	<label for="rex-a256-article-id">'. $I18N->msg('search_article_id') .'</label>
	        <input type="text" name="a256_article_id" id="rex-a256-article-id"'. rex_tabindex() .' />
    	    <input class="rex-form-submit" type="submit" name="a256_start_search" value="'. $I18N->msg('search_start') .'"'. rex_tabindex() .' />
		    </div>

    		<div class="rex-fl-rght">
    			<label for="rex-a256-category-id">'. $I18N->msg('search_quick_navi') .'</label>';

    			$category_select->setAttribute('tabindex', rex_tabindex(false));

  $form .= $category_select->get() .'
    			<noscript>
    			  <input type="submit" name="a256_start_jump" value="'. $I18N->msg('search_jump_to_category') .'" />
    			</noscript>
        </div>
        </fieldset>
      </form>';
