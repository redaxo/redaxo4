...
class rexradio{
	...
	function out(){
		...
		*/
		//$ausgabe = "<fieldset>";
		$type = "radio";
		if ($this->select_multiple) $type = "checkbox";
		for ($i=0;$i<$this->counter;$i++)
		{
			$ausgabe .= '<p><input type="'.$type.'" id="lb_'.$i.'" name="'.$this->select_name.'" value="'.$this->option_value[$i].'" ';
			for ($j=0;$j<$this->option_anzahl;$j++)
			{
				if ($this->option_selected[$j] == $this->option_value[$i])
				{
			...
			$ausgabe .= '/><label for="lb_'.$i.'" >'.$this->option_name[$i].'</label>';
		}
		// $ausgabe .= "</fieldset>";
		return $ausgabe;	
	}
