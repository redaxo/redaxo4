...
title("<a href=index.php?page=community class=head>Community</a>: <a href=index.php?page=community&subpage=comment class=head>Artikelkommentarverwaltung </a>","");
...
echo "	<table border=0 cellpadding=5 cellspacing=1 width=770>
	...
	<tr>
		<td width=150 class=grey>Kommentar</td>
		<td align=left class=dgrey><input type=text size=10 style='width:100%' name=searchtxt value=\"$searchtxt\"></td>
		<td align=left width=200 class=dgrey><select name=comstatus size=1 style='width:100%'>";
...
echo "</select></td>
		<td align=left class=grey width=150><input type=submit value='Suche starten'></td>
	</tr>
	</form>
	</table><br>";
...
echo "	<table border=0 cellpadding=5 cellspacing=1 width=770>
	...
	<tr>
		<td class=dgrey width=15%>Username</td>
		<td class=dgrey width=15%>Artikelname</td>
		<td class=dgrey width=10%>Stamp</td>
		<td class=dgrey width=60%>Kommentar</td>
	</tr>";
...
for($i=0;$i<$COMMENTS->getRows();$i++)
{
	...
	echo "
		<tr>
			<td class=grey valign=top><a href=index.php?page=community&subpage=user&user_id=".$COMMENTS->getValue("rex__user.id").">".htmlentities($COMMENTS->getValue("rex__user.login"))."</a></td>
			<td class=grey valign=top><a href=/index.php?article_id=".$COMMENTS->getValue("rex_article.id")." target=_blank>".htmlentities($COMMENTS->getValue("rex_article.name"))."</a></td>
			<td class=grey valign=top>".date("d. M Y - H:i",$COMMENTS->getValue("rex__article_comment.stamp"))."</td>
			<td class=grey>".nl2br(htmlentities($COMMENTS->getValue("rex__article_comment.comment")))."<br>
