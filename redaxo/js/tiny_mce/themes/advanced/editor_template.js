...
function TinyMCE_advanced_getButtonHTML(button_name) {
	switch (button_name) {
		...
		case "linkInt":
			return '<img id="{$editor_id}_link" src="{$themeurl}/images/linkInt.gif" title="{$lang_linkInt_desc}" width="20" height="20" class="mceButtonNormal" onmouseover="tinyMCE.switchClass(this,\'mceButtonOver\');" onmouseout="tinyMCE.restoreClass(this);" onmousedown="tinyMCE.restoreAndSwitchClass(this,\'mceButtonDown\');" onclick="tinyMCE.execCommand(\'mceLinkInt\', true)">';
		...
		case "email":
			return '<img src="{$themeurl}/images/email.gif" title="{$lang_insert_email}" width="20" height="20" class="mceButtonNormal" onmouseover="tinyMCE.switchClass(this,\'mceButtonOver\');" onmouseout="tinyMCE.restoreClass(this);" onmousedown="tinyMCE.restoreAndSwitchClass(this,\'mceButtonDown\');" onclick="tinyMCEEmail()">';
		...
		case "pasteText":
			return '<img id="{$editor_id}_code" src="{$themeurl}/images/pasteText.gif" title="{$lang_theme_paste_title}" width="20" height="20" class="mceButtonNormal" onmouseover="tinyMCE.switchClass(this,\'mceButtonOver\');" onmouseout="tinyMCE.restoreClass(this);" onmousedown="tinyMCE.restoreAndSwitchClass(this,\'mceButtonDown\');" onclick="TinyMCE_advanced_pasteText();">';
...
function TinyMCE_advanced_getEditorTemplate(settings) {
	...
	// Render row 1
	var buttonNamesRow1 = tinyMCE.getParam("theme_advanced_buttons1", "styleselect,separator,bold,italic,underline,strikethrough,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,bullist,numlist,separator,outdent,indent,separator,").split(',');
	...
	// Render row 2
	var buttonNamesRow2 = tinyMCE.getParam("theme_advanced_buttons2", "undo,redo,separator,linkInt,link,unlink,email,separator,image,cleanup,removeformat,pasteText,code").split(',');
	...
	/*
	// Render row 3
	var buttonNamesRow3 = tinyMCE.getParam("theme_advanced_buttons3", "table,separator,row_before,row_after,delete_row,separator,col_before,col_after,delete_col,separator,hr,separator,sub,sup").split(',');
...
function TinyMCE_advanced_openHTMLSourceEditor() {
	...
	template['html'] = '<html><head><title>{$lang_theme_code_title}</title>\
		...
		<textarea id="htmlSource" name="htmlSource" cols="60" rows="15" style="width:99%; height: 85%">' + tinyMCE.getContent() + '</textarea><br>\
...
// Custom HTML editor function
function TinyMCE_advanced_pasteText() {
	// Alert if the editor isn't selected.
	if (tinyMCE.getContent() == null) {
		if (tinyMCE.settings['focus_alert'])
			alert(tinyMCELang['lang_focus_alert']);
	...
	template['html'] = '<html><head><title>{$lang_theme_paste_title}</title>\
		<link href="{$css}" rel="stylesheet" type="text/css">\
		<script language="javascript">\
		function saveContent() {\
			if (window.opener) {\
				window.opener.tinyMCE.execCommand("mceInsertContent", false, document.getElementById(\'htmlSource\').value.replace(/\\\n/g,\'<br>\') );\
				window.close();\
			}\
		}\
		window.focus();\
		...
		<body>\
		<div class="title">{$lang_theme_paste_title}</div><br>\
		<textarea id="htmlSource" name="htmlSource" cols="60" rows="15" style="width:99%; height: 85%"></textarea><br>\
		<input type="button" name="Button" value="{$lang_theme_code_save}" onclick="saveContent();">\
		</body></html>';
