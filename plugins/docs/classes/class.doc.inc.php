	...
	function parseContentHTML($content){
        ...
        if(eregi("{ilink",$content)){
            ...
            for($c=0;$c<count($match[0]);$c++){
                ...
                $content = str_replace($match[0][$c],"<a href=main.php?doc=".$_GET[doc]."&lang=".$this->Lang."&chapter=".$url[0]."#title".$url[1]."><u>".$match[2][$c]."</u></a>",$content);
...
*/
        if(eregi("{bold}",$content)){
            ...
            for($c=0;$c<count($match[0]);$c++){
               $content = str_replace($match[0][$c],"<b>".$match[1][$c]."</b>",$content);
            }

        }        

      if(eregi("{italic}",$content)){

            preg_match_all("/{italic}(.*){\/italic}/Uism",$content,$match);
